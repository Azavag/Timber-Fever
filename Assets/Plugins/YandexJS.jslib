...
RateGameExtern: function () {  
  ...
  CheckSdkReady: function()
  {
    if(sdkReady)
    {
        ...
        myGameInstance.SendMessage('SceneSwitcher', 'SwitchScene'); 
    } 
  },
  ...
  ShowRewardedAdvExtern: function(){
    ...
    ysdk.adv.showRewardedVideo({
      callbacks: {
        onOpen: () => {
          myGameInstance.SendMessage("LevelLoader", "PauseGame");         
          console.log('Video ad open.');
        },
        onRewarded: () => {
          myGameInstance.SendMessage("BonusSystem", "GetReadyForReward");                
        },
        onClose: () => {
          myGameInstance.SendMessage("LevelLoader", "UnpauseGame");   
          myGameInstance.SendMessage("BonusSystem", "GetBonus");
          console.log('Video ad closed');
        }, 
        onError: (e) => {
          myGameInstance.SendMessage("LevelLoader", "NotReadyForReward");     
          console.log('Error while open BonusSystem ad:', e);
        }
      }
    })
  },
